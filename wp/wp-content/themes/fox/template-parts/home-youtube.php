...
global $fox_options;
?>
<article class="youtube-card <?php if(is_sticky()){echo 'post-sticky';} ?>">
	<!-- Hình đại diện thay thế -->
	<?php if ( has_post_thumbnail() && empty(get_post_meta( $post->ID, 'photo1', true )) && empty(get_post_meta( $post->ID, 'youtube1', true ))) { ?>
	<div class="youtube-hinh">
	...
	<?php } else  if(!empty(fox_anh_dai_dien()) && empty(get_post_meta( $post->ID, 'photo1', true )) && empty(get_post_meta( $post->ID, 'youtube1', true ))) { ?>
	<div class="youtube-hinh">
	...
	<?php if ( 'post' === get_post_type() || (is_array(get_the_terms( $post->ID, 'channel')) && !empty(get_the_terms( $post->ID, 'channel')))){echo '<div style="margin-top:65px;"></div>';} ?>
	<div class="codex-footer">
		<?php if ( 'post' === get_post_type() ) : $category = get_the_category(); $category = reset( $category ); ?>
		<div class="youtube-cm"><a  href="<?php echo esc_url( get_category_link( $category ) ); ?>"><i class="fa-regular fa-bolt"></i> <?php echo esc_html( $category->name ); ?></a></div>
		<?php endif;  ?>
		<?php if(is_array(get_the_terms( $post->ID, 'channel')) && !empty(get_the_terms( $post->ID, 'channel'))){ echo '<div class="youtube-cm">'; the_terms( $post->ID, 'channel', '<i class="fa-regular fa-bolt"></i> '); echo '</div>'; } ?>
