...
<div class="author-site-bg">
<?php if(!empty(get_the_author_meta('first_name')) || !empty(get_the_author_meta('last_name'))){ ?>
<div class="author-site-name"><?php echo get_the_author_meta('last_name') .' '. get_the_author_meta('first_name'); ?></div>
<?php } ?>
<ul class="circles">
                    <li></li>
                    <li></li>
...
<div><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>">
<img width="150" height="150" class="lazyload" <?php global $fox_options; if(isset($fox_options['speed1']) && isset($fox_options['speed2'])){echo 'src="'.get_template_directory_uri().'/images/anh-dai-dien.png" data-';} ?>src="<?php echo esc_url( get_avatar_url( get_the_author_meta( 'ID' ), ['size' => '150'])); ?>" /></a></div>
<?php if(!empty(get_the_author_meta('first_name')) || !empty(get_the_author_meta('last_name'))){ ?>
<div class="author-name"><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>"><?php echo get_the_author_meta('last_name') .' '. get_the_author_meta('first_name'); ?></a></div><?php } ?>
